...
export default function Serveur() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(() => toast.error("Erreur de chargement des données", { position: "top-right" }));
      ...
      for (const tableId in newTables) {
        ...
        if (next.status === "bill" && next.serverId === user.id) {
          ...
          toast.warning(`💰 Table ${tableId} demande l'addition`, {
            position: "top-right", autoClose: false, toastId: `bill-${tableId}`,
          });
        }
      }
    ...
    socket.on("order-ready", ({ tableId }) => {
      ...
      toast.success(`🍳 Commande prête — Table ${tableId}`, {
        position: "top-right", autoClose: false, toastId: `ready-${tableId}`,
      });
      setTables((prev) => ({
        ...prev,
  ...
  // ── Optimistic update ───────────────────────────────────────────────────
  const applyOptimistic = useCallback((tableId, action) => {
    const clearsTable = action === "paid" || action === "cancel";
    if (clearsTable) {
      setOrders((prev) => { const c = { ...prev }; delete c[tableId]; return c; });
    ...
    setTables((prev) => ({
      ...
      [tableId]: {
        ...
        status:
          ...
          action === "served"  ? "notPayed"  :
          clearsTable          ? "empty"      :
          prev[tableId]?.status,
        serverName: clearsTable ? null : action === "confirm" ? user?.username : prev[tableId]?.serverName,
        serverId:   clearsTable ? null : action === "confirm" ? user?.id       : prev[tableId]?.serverId,
      },
    }));
  }, [user]);
  ...
  const doAction = async (tableId, action) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        fetch(`${API_BASE_URL}/tables`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const getItemCount = (tableId) => {
    ...
    if (!o) return 0;
    return Object.values(o).reduce((s, items) => s + items.reduce((a, i) => a + i.qt, 0), 0);
  };

  const getOrderTotal = (tableId) => {
    const o = orders[tableId]?.order;
    if (!o) return 0;
    return Object.values(o).reduce((s, items) => s + items.reduce((a, i) => a + i.price * i.qt, 0), 0);
  };
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      myTables: vals.filter(t => t.serverId === user?.id && t.status !== "empty").length,
      ready:    vals.filter(t => t.status === "ready"   && t.serverId === user?.id).length,
      bill:     vals.filter(t => t.status === "bill"    && t.serverId === user?.id).length,
    };
  }, [tables, user]);
  ...
  const tabCounts = useMemo(() => {
    const base = Object.values(tables).filter(t =>
      t.status !== "empty" && (ownerFilter === "all" || t.serverId === user?.id)
    );
    return {
      all:       base.length,
      ...
      <main className="main-content">
        ...
        <div className="stats-strip">
          <div
            ...
            onClick={() => setStatusFilter(s => s === "ordered" ? "all" : "ordered")}
          >
          ...
          {stats.ready > 0 && (
            <div
              ...
              onClick={() => { setStatusFilter("ready"); setOwnerFilter("mine"); }}
            >
              <span className="stat-icon">🍳</span>
          ...
          {stats.bill > 0 && (
            <div
              ...
              onClick={() => { setStatusFilter("bill"); setOwnerFilter("mine"); }}
            >
              <span className="stat-icon">💰</span>
        ...
        <div className="filter-section">
          <div className="filter-top">
            ...
            <div className="owner-toggle">
              ...
              >
                Mes tables <span className="toggle-badge">{ownerCounts.mine}</span>
              </button>
            </div>
          </div>
          ...
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              {orders[selectedTable]?.order ? (
                <>
                  {Object.entries(orders[selectedTable].order).map(([cat, items]) => (
                  ...
                  <div className="modal-total">
                    ...
                    <span className="total-amount">{getOrderTotal(selectedTable)} DH</span>
                  </div>
                </>
              ) : (
            ...
            <div className="modal-footer">
              {!isMine ? (
                <div className="claimed-notice">
                  ⚠️ Table prise en charge par <strong style={{ marginLeft: 4 }}>{selData?.serverName}</strong>
                </div>
              ) : (
                <>
                  {selStatus === "ordered" && (
                    <>
                      <button className="action-btn btn-confirm" onClick={() => doAction(selectedTable, "confirm")}>
                        ✅ Confirmer la commande
                      </button>
                      <button className="action-btn btn-cancel" onClick={() => doAction(selectedTable, "cancel")}>
                        Annuler
                      </button>
                    </>
                  )}
                  ...
                  {selStatus === "ready" && (
                    <button className="action-btn btn-served" onClick={() => doAction(selectedTable, "served")}>
                      🍽️ Marquer comme servie
                    </button>
                  )}
                  {(selStatus === "notPayed" || selStatus === "bill") && (
                    <button className="action-btn btn-paid" onClick={() => doAction(selectedTable, "paid")}>
